...
export default async function PortalSACPage({
  searchParams,
}: {
  ...
  const where: { status?: string; clienteId: string } = { clienteId: user.clienteId };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Atendimento estruturado a reclamações com prazo de resposta"
      />

      <div className="rounded-lg border border-agua/30 bg-agua/5 p-4 text-sm text-slate-700">
        💧 <strong>Olá!</strong> Aqui você acompanha as reclamações registradas da sua empresa, com
        urgência, prazo e status do atendimento. Tela de consulta.
      </div>

      <div className="flex flex-wrap gap-2 text-xs">
        <Link href="/portal/sac" className={"rounded-md border px-3 py-1.5 " + (!status ? "border-petroleo bg-petroleo/5 text-petroleo" : "border-slate-200 hover:bg-slate-50")}>
          Todas
        </Link>
        {Object.keys(STATUS_CLASS).map((s) => (
          <Link key={s} href={`/portal/sac?status=${s}`}
