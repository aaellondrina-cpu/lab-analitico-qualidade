...
export default async function PortalAuditoriasPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-agua/30 bg-agua/5 p-4 text-sm text-slate-700">
        💧 <strong>Olá!</strong> Aqui você consulta as auditorias internas e externas, o auditor, a
        norma avaliada e o resultado de cada uma. Tela de consulta.
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {auditorias.map((a) => (
                <tr key={a.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-xs text-slate-700">{a.numero}</td>
...
function FilterLink({
  ...
  const href = value ? `/portal/auditorias?${param}=${value}` : `/portal/auditorias`;
