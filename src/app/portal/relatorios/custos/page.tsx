...
export default async function PortalCustosPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-agua/30 bg-agua/5 p-4 text-sm text-slate-700">
        💧 <strong>Olá!</strong> Aqui você consulta os custos de produção — insumos, embalagens e
        custo por lote/litro — dos últimos 90 dias. Tela de consulta.
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <Link href="/portal/relatorios/custos" className={"rounded-md border px-3 py-1.5 " + (!produtoFiltro ? "border-petroleo bg-petroleo/5 text-petroleo" : "border-slate-200 hover:bg-slate-50")}>
          Todos
        </Link>
        {produtos.map((p) => (
          <Link key={p.id} href={`/portal/relatorios/custos?produto=${p.id}`}
