    ...
    <>
      ...
      {cartas.length === 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-slate-400 text-sm">
          Nenhuma carta calculada. Clique em &quot;Recalcular cartas&quot; após lançar resultados.
        </div>
      ) : (
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3">
                      <Link
                        href={`/cep/${c.produtoId}/${encodeURIComponent(c.parametro)}`}
                        className="font-medium text-petroleo hover:underline"
                      >
                        {c.produto.nome}
