  ...
  return (
    <>
      ...
      <form className="mb-4 flex flex-wrap gap-2 text-xs">
        <Link href="/bpf" className={"rounded-md border px-3 py-1.5 " + (!etapa ? "border-petroleo bg-petroleo/5 text-petroleo" : "border-slate-200 hover:bg-slate-50")}>
          Todas
        </Link>
        {Object.entries(ETAPA_LABEL).map(([k, label]) => (
          <Link key={k} href={`/bpf?etapa=${k}`}
