...
import { TratarNC } from "./_components/TratarNCButton";
...
export default async function NaoConformidadesPage({
  ...
  const canEncerrar = user.role === "ADMIN" || user.role === "RESPONSAVEL_TECNICO";
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {STATUSES.map((s) => {
          ...
          return (
            <Link
              key={s.value}
              href={`/nao-conformidades?status=${s.value}`}
              ...
              <article key={nc.id} className="rounded-lg border border-slate-200 bg-white p-4">
                <header className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-xs text-slate-500">
                    Amostra <Link href={`/amostras`} className="font-mono text-petroleo hover:underline">{nc.amostra.numeroOS}</Link>
                  </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 text-sm">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pt-2">
                      <TratarNC id={nc.id} status={nc.status} canEncerrar={canEncerrar} />
                    </div>
                  </div>
                </div>
              </article>
