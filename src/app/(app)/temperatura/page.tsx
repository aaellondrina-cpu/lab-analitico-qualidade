...
import { LeituraForm } from "./_components/LeituraForm";
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link href="/temperatura/novo-ponto" className="rounded-md bg-petroleo px-4 py-2 text-sm font-medium text-white hover:bg-petroleo-dark">
            + Novo ponto
          </Link>
        }
      />
      ...
      {pontos.length === 0 ? (
        <div className="rounded-lg border border-slate-200 bg-white p-8 text-center text-slate-400 text-sm">
          Cadastre um ponto de monitoramento para começar.
        </div>
      ) : (
        <>
          ...
          {/* Form de leitura rápida */}
          <div className="mb-6 rounded-lg border border-slate-200 bg-white p-4">
            <div className="mb-3 text-xs font-medium text-slate-700">Lançar leitura</div>
            <LeituraForm pontos={pontos.map((p) => ({ id: p.id, nome: p.nome, tempMin: p.tempMin, tempMax: p.tempMax }))} />
          </div>
        </>
      )}
