...
export default async function TreinamentosPage({
  ...
  // Mapa: colaboradorId+documentoId -> último treinamento
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link href="/colaboradores" className="rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
            Colaboradores →
          </Link>
        }
      />
      ...
      <section className="mb-6 rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-petroleo mb-3">Registrar treinamento</h2>
        {colaboradores.length === 0 || documentos.length === 0 ? (
          <p className="text-sm text-slate-500">
            Cadastre ao menos um <Link href="/colaboradores" className="text-petroleo hover:underline">colaborador</Link> e um{" "}
            <Link href="/documentos" className="text-petroleo hover:underline">documento</Link> primeiro.
          </p>
        ) : (
          <TreinamentoForm
            colaboradores={colaboradores.map((c) => ({ id: c.id, nome: c.nome, cargo: c.cargo }))}
            documentos={documentos.map((d) => ({ id: d.id, codigo: d.codigo, titulo: d.titulo, versao: d.versao }))}
          />
        )}
      </section>

      <h2 className="text-sm font-semibold text-petroleo mb-3">Matriz</h2>
      {colaboradores.length === 0 || documentos.length === 0 ? (
