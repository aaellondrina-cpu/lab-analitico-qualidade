...
export default async function AguaPage({
  ...
  // Último status por tipo (para os cards)
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link href="/agua-processo/novo" className="rounded-md bg-petroleo px-4 py-2 text-sm font-medium text-white hover:bg-petroleo-dark">
            + Lançar resultado
          </Link>
        }
      />
      ...
      <form className="mb-4 flex gap-2 text-xs">
        <Link href="/agua-processo" className={"rounded-md border px-3 py-1.5 " + (!tipo ? "border-petroleo bg-petroleo/5 text-petroleo" : "border-slate-200 hover:bg-slate-50")}>
          Todos
        </Link>
        {Object.keys(TIPO_LABEL).map((t) => (
          <Link key={t} href={`/agua-processo?tipo=${t}`}
